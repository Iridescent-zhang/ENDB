...
import com.strumcode.endb.backend.im.BPlusTree;
import com.strumcode.endb.backend.parser.statement.SingleExpression;
 ...
 * 如果这个字段有索引，那个 IndexUID 指向了索引 B+ 树的根，否则该字段为 0。
 */
public class Field {
    long uid;
    private Table tb;
    ...
    // calExp方法将每个单独的条件表达式转换成一个FieldCalRes对象
    public FieldCalRes calExp(SingleExpression exp) throws Exception {
        Object v = null;
        FieldCalRes res = new FieldCalRes();
        switch(exp.compareOp) {
            // 假如 exp.value 是 "10" 且 fieldType 是 "int32"，那么这个条件会转换成区间 [0, 9]。
            case "<":
                res.left = 0;
                ...
                break;
            // 假如 exp.value 是 "10" 且 fieldType 是 "int32"，那么这个条件会转换成区间 [10, 10]。
            case "=":
                v = string2Value(exp.value);
                ...
                break;
            case ">":
            // 假如 exp.value 是 "10" 且 fieldType 是 "int32"，那么这个条件会转换成区间 [11, Long.MAX_VALUE]。
